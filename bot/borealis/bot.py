...
class DiscordBot(discord.Client):
	...
	def do_command(self, msg, words):
		...
		if command == "help":
			...
			count = self.ping_server(b"players", "int")
			...
			count = self.ping_server(b"admins", "int")

			if count == None:
				response = "{0} - Sorry! I was unable to ping the server!".format(msg.author.mention())
			else:
				response = "{0} - There are {1} admins and mods on the server.".format(msg.author.mention(), count)
		elif command == "docount":
			count = self.ping_server(b"docount", "int")
			...
			gamemode = self.ping_server(b"gamemode", "str")
		...
		#Authorized commands:
			...
			if len(words) < 3:
				...
				server_reply = self.ping_server(bytes(to_send, "utf-8"), "str")
			...
			server_reply = self.ping_server(bytes("restart={0}&key={1}".format(msg.author.name, self.server_info["key"]), "utf-8"), "str")
			...
			if len(words) < 2:
				...
				server_reply = self.ping_server(bytes("announce={0}&key={1}&msg={2}".format(msg.author.name, self.server_info["key"], message), "utf-8"), "str")
			...
			server_reply = self.ping_server(bytes("faxlist={0}&key={1}".format(received, self.server_info["key"]), "utf-8"), "list")

			if server_reply == None or len(server_reply) == 0:
				response = "Sorry! I couldn't execute the command for whatever reason!"
			else:
				response = "Got a reply back from the server! It's a long one!"
			...
			if len(words) < 3:
				...
				fax_id = words[1]
				server_reply = self.ping_server(bytes("getfax={0}&key={1}&received={2}".format(fax_id, self.server_info["key"], received), "utf-8"), "str")

				if server_reply == None:
					response = "Sorry! I couldn't execute the command for whatever reason!"
				else:
					response = "Got a reply back from the server! Fax content as follows:\n\n```"
	...
	def ping_server(self, question, expect_response):
		try:
			...
			if response != None:
				...
				if expect_response == "int":
					return response
				elif expect_response == "str":
