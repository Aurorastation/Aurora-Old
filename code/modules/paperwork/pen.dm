...
//	M << "\red You feel a tiny prick!" //That's a whole lot of meta!
	...
	msg_admin_attack("[key_name_admin(user)] Used the [name] to stab [key_name_admin(M)] (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)")
	return
