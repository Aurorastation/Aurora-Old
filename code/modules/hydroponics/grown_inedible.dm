...
/obj/item/weapon/grown/nettle/death/attack(mob/living/carbon/M as mob, mob/user as mob)
	...
	if(istype(M, /mob/living))
		...
		msg_admin_attack("[key_name_admin(user)] used the [src.name] on [key_name_admin(M)] (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)")
