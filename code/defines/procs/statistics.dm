proc/sql_poll_players()
	...
	if(!dbcon.IsConnected())
		...
		var/DBQuery/query = dbcon.NewQuery("INSERT INTO ss13_population (playercount, time) VALUES ([playercount], '[sqltime]')")
		if(!query.Execute())
...
proc/sql_poll_admins()
	...
	if(!dbcon.IsConnected())
		...
		var/sqltime = time2text(world.realtime, "YYYY-MM-DD hh:mm:ss")
		var/DBQuery/query = dbcon.NewQuery("INSERT INTO ss13_population (admincount, time) VALUES ([admincount], '[sqltime]')")
		if(!query.Execute())
			var/err = query.ErrorMsg()
