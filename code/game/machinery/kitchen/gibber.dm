...
/obj/machinery/gibber/proc/startgibbing(mob/user as mob)
	...
	if( istype(src.occupant, /mob/living/carbon/human/) )
		...
		msg_admin_attack("[key_name_admin(user.ckey)] gibbed [key_name_admin(src.occupant)] (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)")
		...
		if(src.occupant.client) // Gibbed a cow with a client in it? log that shit
			...
			user.attack_log += "\[[time_stamp()]\] Gibbed <b>[src.occupant]/[src.occupant.ckey]</b>"
			msg_admin_attack("[key_name_admin(user)] gibbed [key_name_admin(src.occupant)] - <A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[src.x];Y=[src.y];Z=[src.z]'>JMP</a>")
