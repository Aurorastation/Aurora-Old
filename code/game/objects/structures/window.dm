		...
		if (istype(G.affecting, /mob/living))
			...
			switch (state)
				if(1)
					...
					msg_admin_attack("[key_name_admin(user)] slams [key_name_admin(M)]'s against \the [src]! - <A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[M.x];Y=[M.y];Z=[M.z]'>JMP</a>")
				if(2)
					if (prob(50))
						M.Weaken(1)
					M.apply_damage(10)
					...
					msg_admin_attack("[key_name_admin(user)] bashes [key_name_admin(M)]'s against \the [src]! - <A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[M.x];Y=[M.y];Z=[M.z]'>JMP</a>")
				if(3)
					M.Weaken(5)
					M.apply_damage(20)
					hit(50)
					visible_message("\red <big><b>[user] crushes [M] against \the [src]!</b></big>")
					msg_admin_attack("[key_name_admin(user)] crushes [key_name_admin(M)]'s against \the [src]! - <A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[M.x];Y=[M.y];Z=[M.z]'>JMP</a>")
			return
