...
/obj/structure/stool/bed/chair/wheelchair/Bump(atom/A)
	...
	if(propelled || (pulling && (pulling.a_intent == "hurt")))
		...
		if(pulling)
			...
			msg_admin_attack("[key_name_admin(pulling)] has thrusted [key_name_admin(occupant)]'s [name] into \a [A] (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[pulling.x];Y=[pulling.y];Z=[pulling.z]'>JMP</a>)")
		else
