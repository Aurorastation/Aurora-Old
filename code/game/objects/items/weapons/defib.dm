...
/obj/item/weapon/melee/defibrillator/attack(mob/living/carbon/human/M as mob, mob/user as mob)
	...
	if(status)
		if(user.a_intent == "hurt" && emagged)
			...
			msg_admin_attack("<font color='red'>[key_name_admin(user)] defibrillated [key_name_admin(H)] with [src.name]</font> - <A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[H.x];Y=[H.y];Z=[H.z]'>JMP</a>" )
			if(!iscarbon(user))
				M.LAssailant = null
			else
				M.LAssailant = user
		...
		if(do_after(user, 10))
			if(H.stat == 2 || H.stat == DEAD)
				...
				if(H.health > -100 && !(temp.status & ORGAN_DESTROYED) && !H.suiciding && (M.has_brain()) && toolate(M)) //!(M_NOCLONE in H.mutations) && We don't have this.
