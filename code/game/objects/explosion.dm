...
// 3/7/14 will calculate to 80 + 35
		...
		for(var/mob/M in player_list) // What on made you decide to scan the world? //for(var/mob/M in world)
			if(M.z == epicenter.z)
				if(!(M in close))
		...
		if(adminlog)
			...
			log_game("Explosion with size ([devastation_range], [heavy_impact_range], [light_impact_range]) in area [epicenter.loc.name] (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[epicenter.x];Y=[epicenter.y];Z=[epicenter.z]'>JMP</a>)")
...
//			explotionCtrl.addTurfToGetHit(T, dist)
			T.ex_act(dist)
			if(T)
